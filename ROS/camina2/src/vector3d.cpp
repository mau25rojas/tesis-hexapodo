...
{
 	x = v.x;	y=v.y;	z=v.z; //asigno cada uno de los elementos del punto los mismo elementos del vector
}

punto3d::punto3d(float nx, float ny, float nz) // Constructor con argumentos de la clase punto3d.
{
	x = nx;	y = ny;	z = nz; //asigno nuevos valores individualmente (3x float)
}
...
{
	 x= p.x;	 y= p.y;	 z= p.z; //asigno cada uno de los elementos de punto3d al vector3d
}

vector3d::vector3d(punto3d a, punto3d b)  //constructor de vector a partir de 2 puntos
{
	//el vector resultante va desde 'a' hasta 'b'
	x = b.x-a.x;	y = b.y-a.y;	z = b.z-a.z; //el vector lo obtengo por resta de cada componente
}

vector3d::vector3d(float nx,float ny,float nz)	//constructor de vector a partir de las 3 coordenadas
{
	x = nx;	y = ny; 	z= nz; //asigno nuevos valores de cada una de las componentes del vector
}
...
{
	punto = p;   vector = v; //asignacion directa del vector y del punto
}
...
int recta3d::alabeada(recta3d r) // determina si dos rectas son alabeadas
{
