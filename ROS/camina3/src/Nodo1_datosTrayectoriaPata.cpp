...
#include "camina3/PlanificadorParametros.h"
#include "camina3/SenalesCambios.h"
// Used API services:
#include "vrep_common/VrepInfo.h"
//Clientes y Servicios
ros::ServiceClient client_Planificador;
camina3::PlanificadorParametros srv_Planificador;

// variables Globales
bool simulationRunning=true;
bool sensorTrigger=false;
float simulationTime=0.0f;
camina3::DatosTrayectoriaPata datosTrayectoriaPata;
...
bool InicioApoyo=false, FinApoyo=false;
float delta_t=0.0;
FILE *fp1;
ros::Publisher chatter_pub1;
//camina3::SenalesCambios senales;
...
void relojCallback(camina3::SenalesCambios msgSenal)
{
    if (!msgSenal.Stop){

        if (t_aux<2*(T/divisionTrayectoriaPata) and FinApoyo) {
//            ROS_INFO("t_aux<=2*(T/divisionTrayectoriaPata): %f<=%f",t_aux,2*(T/divisionTrayectoriaPata));
            InicioApoyo=true;
            FinApoyo=false;
        }

        if (fabs(t_aux-beta*T)<2*(T/divisionTrayectoriaPata)) {
            FinApoyo=true;
//            ROS_INFO("Nodo1::T[%d]: fin_apoyo=true",Tripode);
        }
...
//            ROS_INFO("Nodo1::T[%d] tiempo: %.3f, llamo planificacion",Tripode,t_aux);
            ...
            //-- Se cambia el numero de puntos para seguir con la misma frecuencia
...
//        t_aux=delta_t+desfasaje_t*T;
        ...
        fprintf(fp1,"%.3f\t%.3f\t%.3f\t%.3f\n",simulationTime,T,divisionTrayectoriaPata,t_aux);

        if (fabs(delta_t-T)<2*(T/divisionTrayectoriaPata)) {
            delta_t = 0.0;
        }

//        delta_t = delta_t + T/divisionTrayectoriaPata;
        t_aux = t_aux + T/divisionTrayectoriaPata;
    }
}
...
//-- Manda topico especifico para cada pata
    ...
    topicName+=Id;
    chatter_pub1=node.advertise<camina3::DatosTrayectoriaPata>(topicName.c_str(), 100);
//-- Clientes y Servicios
    client_Planificador = node.serviceClient<camina3::PlanificadorParametros>("PlanificadorPisada");
//-- Log de datos
    std::string fileName("../fuerte_workspace/sandbox/TesisMaureen/ROS/camina3/datos/SalidaDatos");
