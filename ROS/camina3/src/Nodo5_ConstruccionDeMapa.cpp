...
camina3::EspacioTrabajoParametros srv_EspacioTrabajo;

// Global variables (modified by topic subscribers):
bool simulationRunning=true;
bool sensorTrigger=false;
float simulationTime=0.0f;
camina3::InfoMapa infoMapa;
int matrizMapa[100][100];
bool bool_matrizMapa[100][100];
int nCeldas_i=0, nCeldas_j=0;
int cantidadObstaculos=0;
int coordenadaAjuste_i[Npatas/2]={0,0,0}, coordenadaAjuste_j[Npatas/2]={0,0,0}, coordenadaPreAjuste_i[Npatas]={0,0,0}, coordenadaPreAjuste_j[Npatas/2]={0,0,0}, pataID[Npatas/2]={0,0,0};
...
void ubicacionRobCallback(camina3::UbicacionRobot msgUbicacionRobot)
{
...
void ajusteCallback(camina3::InfoMapa msgInfoMapa)
{
    ...
    for(int k=0; k<Npatas/2; k++) {
        matrizMapa[coordenadaPreAjuste_i[k]][coordenadaPreAjuste_j[k]]=pataID[k]+1;
        matrizMapa[coordenadaAjuste_i[k]][coordenadaAjuste_j[k]]='a';
        fprintf(fp2,"\n[%d]Preajuste:i=%d,j=%d - ajuste:i=%d,j=%d",pataID[k]+1,coordenadaPreAjuste_i[k],coordenadaPreAjuste_j[k],coordenadaAjuste_i[k],coordenadaAjuste_j[k]);
//            ROS_INFO("Nodo5: Posicion actual:[%d] i:%d\t j:%d",k+1,coordenadaPreAjuste_i[k],coordenadaPreAjuste_j[k]);
    ...
    for(int k=0; k<Npatas/2; k++) {
        if(bool_matrizMapa[coordenadaPreAjuste_i[k]][coordenadaPreAjuste_j[k]]){
            fprintf(fp2,"\nPata[%d] Coincide Obstaculo: [%d][%d]",pataID[k]+1,coordenadaPreAjuste_i[k],coordenadaPreAjuste_j[k]);
            ROS_WARN("Nodo5: Pata[%d] coincide con obstaculo[%d][%d]",pataID[k]+1,coordenadaPreAjuste_i[k],coordenadaPreAjuste_j[k]);
        }
    }
...
//-- Topicos susbcritos y publicados
    ...
    chatter_pub = node.advertise<camina3::InfoMapa>("GraficaMapa", 100);
    //Clientes y Servicios
//    client_EspacioTrabajo=node.serviceClient<camina3::EspacioTrabajoParametros>("EspacioTrabajo");
...
//--- Inicializacion de archivo de salida
    fp1 = fopen("/home/maureenr/fuerte_workspace/sandbox/TesisMaureen/ROS/camina3/datos/SalidaMapa_robot.txt","w+");
    fp2 = fopen("/home/maureenr/fuerte_workspace/sandbox/TesisMaureen/ROS/camina3/datos/SalidaMapa_ajuste.txt","w+");
//--- Ciclo de ROS
//	periodo=1.5;
//    f=1/periodo;
...
void Limpiar_matrizMapa(int nCeldas_i, int nCeldas_j, int cantidadObstaculos){
