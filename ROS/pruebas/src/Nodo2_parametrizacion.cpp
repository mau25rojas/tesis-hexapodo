...
#include "pruebas/CinversaParametros.h"
// Used API services:
...
pruebas::CinversaParametros srv_Cinversa;
...
*/
void datosCallback(const pruebas::DatosTrayectoriaPata msg_datoTrayectoria)
{
...
//-- Topicos susbcritos y publicados
    chatter_pub = node.advertise<pruebas::AngulosMotor>("DatosDeMotores", 100);
    ros::Subscriber subInfo = node.subscribe("/vrep/info",1,infoCallback);
//-- Recibe topico especifico
    ros::Subscriber sub = node.subscribe("datosTrayectoria", 100, datosCallback);
//-- Clientes y Servicios
    client_Cinversa = node.serviceClient<pruebas::CinversaParametros>("Cinversa");

    std::string fileName("../fuerte_workspace/sandbox/TesisMaureen/ROS/pruebas/datos/QXEnviada_Pata");
