...
float posicionInicio_x,posicionInicio_y,posicionInicio_z;
...
void datosCallback(camina::DatosTrayectoriaPata msgDatosTrayectoriaPata){
    ...
    if (finTrayectoria) {
        ...
        if (client_simRosGetObjectPose.call(srv_simRosGetObjectPose)&&(srv_simRosGetObjectPose.response.result!=-1)){
            ...
            posicionInicio_x = PataTipPose.pose.position.x;
            posicionInicio_y = PataTipPose.pose.position.y;
//            posicionInicio_z = PataTipPose.pose.position.z;
            posicionInicio_z = z_Offset;
//            ROS_INFO("actual: [%d] x=%.3f, y=%.3f, z=%.3f",Npata_arg,posicionInicio_x,posicionInicio_y,posicionInicio_z);
        } else {
...
//            if (Npata_arg==6) ROS_INFO("inicio: [%d] x=%.3f\ty=%.3f\tz=%.3f",Npata_arg,x_S0,y_S0,z_S0);
        ...
        {   posicionInicio_x = srv_TransTrayectoria_PataMundo.response.x_Mundo;
            posicionInicio_y = srv_TransTrayectoria_PataMundo.response.y_Mundo;
            posicionInicio_z = srv_TransTrayectoria_PataMundo.response.z_Mundo;
            x_FinApoyo = srv_TransTrayectoria_PataMundo.response.x_Mundo;
            y_FinApoyo = srv_TransTrayectoria_PataMundo.response.y_Mundo;
            z_FinApoyo = srv_TransTrayectoria_PataMundo.response.z_Mundo;
//             ROS_INFO("inicio: \n[%d] posicionInicio_x=%.3f\tposicionInicio_y=%.3f\tposicionInicio_z=%.3f",Npata_arg,posicionInicio_x,posicionInicio_y,posicionInicio_z);
        } else {
    ...
    if (0<=t_Trayectoria && t_Trayectoria<beta){
        Trayectoria_FaseApoyo(t_Trayectoria,posicionInicio_x,posicionInicio_y,posicionInicio_z);
    } else {
        Trayectoria_FaseTrans_Eliptica(t_Trayectoria,x_FinApoyo,y_FinApoyo,z_FinApoyo);
    }
////         Creamos archivo
    fprintf(fp1,"%d,%.3f,%.3f,%.3f\n",Npata_arg,x_Tray,y_Tray,z_Tray);
...
void Trayectoria_FaseApoyo(float t_Trayectoria,float PuntoInicio_x,float PuntoInicio_y,float PuntoInicio_z){
    ...
    x_aux = PuntoInicio_x;
    y_aux = (PuntoInicio_y+lambda_Apoyo/2) - velocidadApoyo*t_Trayectoria;
    z_aux = PuntoInicio_z;
