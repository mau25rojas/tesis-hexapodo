...
bool TrayectoriaMundoPata(camina::TransTrayectoriaParametros::Request  &req,
                        camina::TransTrayectoriaParametros::Response &res)
{
...
//        //--Devolvemos segunda transformacion
//            x_Robot = -(origenPata_x[req.Npata]*cos(rotacionPata[req.Npata])+origenPata_y[req.Npata]*sin(rotacionPata[req.Npata])) + req.x_S0*cos(rotacionPata[req.Npata]) + req.y_S0*sin(rotacionPata[req.Npata]);
//            y_Robot = -(-origenPata_x[req.Npata]*sin(rotacionPata[req.Npata])+origenPata_y[req.Npata]*cos(rotacionPata[req.Npata])) - req.x_S0*sin(rotacionPata[req.Npata]) + req.y_S0*cos(rotacionPata[req.Npata]);
//        //--Devolvemos primera transformacion: del Sistema ROBOT al Sistema MUNDO
//            res.x_Pata = -(req.x_UbicacionRob*cos(req.theta_Rob)+req.y_UbicacionRob*sin(req.theta_Rob)) + x_Robot*cos(req.theta_Rob) + y_Robot*sin(req.theta_Rob);
//            res.y_Pata = -(-req.x_UbicacionRob*sin(req.theta_Rob)+req.y_UbicacionRob*cos(req.theta_Rob)) - x_Robot*sin(req.theta_Rob) + y_Robot*cos(req.theta_Rob);
//            res.z_Pata = req.z_S0;
//        //--Devolvemos segunda transformacion
//            x_Robot = -origenPata_x[req.Npata] + req.x_S0*cos(-rotacionPata[req.Npata]) - req.y_S0*sin(-rotacionPata[req.Npata]);
//            y_Robot = -origenPata_y[req.Npata] + req.x_S0*sin(-rotacionPata[req.Npata]) + req.y_S0*cos(-rotacionPata[req.Npata]);
//        //--Devolvemos primera transformacion: del Sistema ROBOT al Sistema MUNDO
//            res.x_Pata = -req.x_UbicacionRob + x_Robot*cos(-req.theta_Rob) - y_Robot*sin(-req.theta_Rob);
//            res.y_Pata = -req.y_UbicacionRob + x_Robot*sin(-req.theta_Rob) + y_Robot*cos(-req.theta_Rob);
//            res.z_Pata = req.z_S0;
        break;
    }
