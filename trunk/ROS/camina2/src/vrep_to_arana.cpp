...
//        loop_rate.sleep();
        ...
        {
        //-- Obtener posiciones en radianes
            Motores = srv_GetJointStatesPublisher.response.state;

//			for(int k=0;k<18;k++){
//			printf("[%d] ",k);
//			std::cout << Motores.name[k] << std::endl;
//			}
...
//			pata[5].Q3 = Motores.position[17]*180/pi;
            ...
            for(int i=0; i<Npatas; i++){
                pata.Q1[i]=Motores.position[i*3+0]*180/pi;
//                pata.Q2[0]=Motores.position[i*3+1]*180/pi;
//                pata.Q3[i]=-Motores.position[i*3+2]*180/pi;
            }
        //-- En robot real los motores estan al reves respecto a la simulacion
            pata.Q2[0]=Motores.position[1]*180/pi;
            pata.Q2[1]=-Motores.position[4]*180/pi;
            pata.Q2[2]=Motores.position[7]*180/pi;
            pata.Q2[3]=-Motores.position[10]*180/pi;
            pata.Q2[4]=Motores.position[13]*180/pi;
            pata.Q2[5]=-Motores.position[16]*180/pi;

            pata.Q3[0]=Motores.position[2]*180/pi;
            pata.Q3[1]=Motores.position[5]*180/pi;
            pata.Q3[2]=Motores.position[8]*180/pi;
            pata.Q3[3]=Motores.position[11]*180/pi;
            pata.Q3[4]=Motores.position[14]*180/pi;
            pata.Q3[5]=Motores.position[17]*180/pi;
