...
//             ROS_INFO("inicio: \n[%d] PuntoApoyo_x=%.3f\tPuntoApoyo_y=%.3f\tPuntoApoyo_z=%.3f",Npata_arg,PuntoApoyo_x,PuntoApoyo_y,PuntoApoyo_z);
    ...
    if (t_Trayectoria>=1) {
        ROS_ERROR("Error fatal: t_Trayectoria>1");
        ROS_ERROR("Shutdown de nodo1::pata[%d]!",Npata_arg);
        fclose(fp1);
        ros::shutdown();
        return;
    }
...
////         Creamos archivo
    fprintf(fp1,"%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",Npata_arg,t_Trayectoria,x_S1,y_S1,z_S1,x_Tray,y_Tray,z_Tray);
